...
model = PB.create_model_from_config(
    ...
    modelpars=Dict(
        # "CGconstant"=>false,
        "CGconstant"=>true,  # fix slow drift resulting in final eqb point not matching model output
        # "Aconstant"=>true,
    )
)
...
expts_table = [
    ...
    # ("preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate0", [("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
    #      ("CO2pulse", [0, 5e7, 5e7+2.0e7, 1e8], [0, 0, 0, 0])]),

    # # slow degassing change -> steady state changes
    ("POA 16", "preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate1", [("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
         ("CO2pulse", [0, 0.5e7, 0.5e7+2.0e7, 1e8], [0, 0, -2.5e12, -2.5e12]), ("OPAinit", (0.75*3.7e19, 3.98*3.1e15, 4.06*3.193e18))]), 

    # # ("preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate2", [("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
    # #      ("CO2pulse", [0, 5e7, 5e7+1.0e7, 1e8], [0, 0, -2.5e12, -2.5e12])]), 

    ("POA 17", "preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate3", [("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
         ("CO2pulse", [0, 0.5e7, 0.5e7+0.5e7, 1e8], [0, 0, -2.5e12, -2.5e12]), ("OPAinit", (0.75*3.7e19, 3.98*3.1e15, 4.06*3.193e18))]), 

    # # ("preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate4", [("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
    # #      ("CO2pulse", [0, 5e7, 5e7+0.1e7, 1e8], [0, 0, -2.5e12, -2.5e12])]),

    # # fast degassing change -> excite OAE
    ("POA 18", "preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate5", [("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
...
for (expt_id, fileroot, vector_pars) in expts_table
    ...
    # (fileroot, vector_pars) = expts_table[1]
        ...
        P_O_A_U_columns_Table4_4[fileroot] = 
...
linestyles = [:dash, :dashdot, :solid, :dot, :dashdotdot]
...
(; expt_id, paleorun, modeldata, t_ts, A_ts, P_ts, O_ts, A_val, O_val,
                ...
                dPdt_line_const_O, dAdt_line_const_O, dPdt_linesegs_const_O,
                folds_dPdt_lines_POA, dOdt_dPdt_linesegs_POA, dAdt_dPdt_linesegs_POA, dAdt_dPdt_linesegs_POA_end, eqb_point, eqb_point_end) = 
    P_O_A_U_columns_Table4_4["preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate5"]
    ...
    GLMakie.surface!(
        axs, dAdt_surf_P_end, dAdt_surf_O_end, dAdt_surf_A_end;
        # color=(:black, 0.1), # (color, alpha)
        color=fill((:black, 0.1), size(dAdt_surf_P)...),  # workaround for a Makie bug see https://discourse.julialang.org/t/colored-surface-in-makie/61009/4
        transparency=true,
        shading=true,
    ...
    # add intersection lines of A_nullcline and slow-manifold surface
    SolverFunctionsOOEOAE2.plot_segments!(
        GLMakie.lines!, axs, dAdt_dPdt_linesegs_POA_end, [1, 2, 3], (:dash, :solid), nothing;
        color=:black, linewidth=glm_linewidth,
    )
...
for (i, exptroot) in enumerate(["rate1", "rate3", "rate5"])
    local (; A_ts, P_ts, O_ts) = 
    P_O_A_U_columns_Table4_4["preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_"*exptroot]
    GLMakie.lines!(axs, P_ts, O_ts, A_ts; color=:green, linewidth=2, linestyle=linestyles[i])
end
GLMakie.save(joinpath(output_figures_dir, "Rate_CO2pulse_summary_3D_$output_figures_datestamp.png"), fig; px_per_unit=5,)
...
savefig(p_sum, joinpath(output_figures_dir, "Rate_CO2pulse_summary_$output_figures_datestamp.svg"))


# # (paleorun, _, A_ts, P_ts, O_ts, _, _, _, _, _,Obal_P, Obal_O, _, _, _, _, _, _, _) = 
# #     P_O_A_U_columns_Table4_4["preCambrian_Bergman_sharp_switch_unstable5_Corgb7_Psilw_only_rate1"]
# #     println("O=$(O_ts[end]), P=$(P_ts[end]), A=$(A_ts[end])")
