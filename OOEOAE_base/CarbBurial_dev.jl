module CarbBurial_dev

import PALEOboxes as PB
using PALEOboxes.DocStrings

"""
    ReactionCarbBurialAlk_dev
...
$(METHODS_DO)
"""
Base.@kwdef mutable struct ReactionCarbBurialAlk_dev{P} <: PB.AbstractReaction
    base::PB.ReactionBase

    pars::P = PB.ParametersTuple(
        PB.ParDouble("k_TAlk_mccb", 0.5, units="",
            description="mccb is dependent only on TAlk input with this factor"),
        # Isotopes
...
function PB.register_methods!(rj::ReactionCarbBurialAlk_dev)
    ...
    vars = PB.VariableReaction[      
        ...
        # burial
        PB.VarProp("mccb",   "molC/yr",     "Carbonate burial"),
        PB.VarContribScalar("ocean.oceanfloor.DIC_sms", "molC/yr", "DIC reservoir source - sink",
            attributes=(:field_data=>CIsotopeType,)),
        PB.VarContrib("(ocean.oceanfloor.CAL_sms)", "mol Ca/yr", "Calcium reservoir source - sink"),    
        PB.VarContrib("fluxOceanBurial.flux_Ccarb", "molC/yr", "carbonate burial",
            attributes=(:field_data=>CIsotopeType,)),
    ]
    ...
    if CIsotopeType <: PB.AbstractIsotopeScalar
        append!(vars, [
                ...
                # PB.VarDep("ocean.oceanfloor.DIC_delta",  "per mil",  "d13C ocean DIC"),
                ...
                # C isotopes
                PB.VarProp("ocean.mccb_delta", "per mil", "D13C fractionation of marine calcium carbonate burial"),   
...
function do_carb_burial(m::PB.ReactionMethod,  (vars,), cellrange::PB.AbstractCellRange, deltat)
    ...
    rj = m.reaction
    CIsotopeType = rj.pars.CIsotope[]
    frac_burial = 1/length(cellrange.indices) # per cell !! need to remove this hardcode

    for i in cellrange.indices
        if CIsotopeType <: PB.AbstractIsotopeScalar
            # delta of marine carbonate burial
            vars.mccb_delta[i] = vars.DIC_delta[] + vars.D_mccb_DIC[]
        
            # for diagnostic output only
            vars.D_mccb_A[] = vars.D_oceanDIC_A[] + vars.D_mccb_DIC[]
        end
    
        vars.mccb[i]                = rj.pars.k_TAlk_mccb.v * vars.flux_TAlk[]*frac_burial  # alkalinity balance # per cell !!
        mccb_isotope                = @PB.isotope_totaldelta(CIsotopeType, vars.mccb[i], vars.mccb_delta[i])
        
        PB.add_if_available(vars.CAL_sms, -vars.mccb[i])
        vars.DIC_sms[]              += -mccb_isotope
        vars.flux_Ccarb[i]          += mccb_isotope
    end
