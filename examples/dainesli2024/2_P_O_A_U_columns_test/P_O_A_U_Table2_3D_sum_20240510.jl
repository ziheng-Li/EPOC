...
output_figures_dir = joinpath(dropbox_output_dir, "P_O_A_U_Table2_3D_sum_Ziheng_20240510")
...
expts_table = [
    ...
    # ("POA 10", "preCambrian_Bergman_Psilw_only", [("k_O2_U", (0.05, 0.75)), ("corg_burial_fac", 0.8), ("OPAinit", (0.1*3.7e19, 2.0*3.1e15, 5.0*3.193e18))]),

    # ("POA 11", "preCambrian_Bergman_sharp_switch_unstable5_Corgb3_Psilw_only", [("k_O2_U", (0.23, 0.25)), ("corg_burial_fac", 1.1), ("OPAinit", (0.32*3.7e19, 1.06*3.1e15, 4.19*3.193e18))]),
    # ("POA 12", "preCambrian_Bergman_sharp_switch_unstable5_Corgb4_Psilw_only", [("k_O2_U", (0.23, 0.25)),("corg_burial_fac", 0.7), ("OPAinit", (0.548*3.7e19, 1.21*3.1e15, 4.53*3.193e18))]),
    # ("POA 13", "preCambrian_Bergman_sharp_switch_unstable5_Corgb6_Psilw_only", [("k_O2_U", (0.23, 0.25)), ("corg_burial_fac", 0.5), ("OPAinit", (0.59*3.7e19, 3.53*3.1e15, 3.83*3.193e18))]),

    ("POA 11-13", "preCambrian_Bergman_sharp_switch_unstable5_change_regimes_Psilw_only", [("k_O2_U", (0.23, 0.25)), 
        ("corg_burial_fac", 1.0), 
        ("set_interp_forcing", "ocean", "force_corg_bf",    [0.0, 0.25e8, 0.25e8+1, 1.5e8, 1.5e8+1, 2e8, 2e8+1, 2.5e8, 2.5e8+1, 3.0e8], [0.2, 0.2, 0.5, 0.5, 0.7,0.7, 1.1, 1.1, 2.2, 2.2]),
        ("OPAinit", (0.1*3.7e19, 3.53*3.1e15, 3.83*3.193e18)
        )]),
...
# (fileroot, vector_pars) = expts_table[1]
    ...
    tspan = (0.0, 3.0e8) # yr # tspan=(-1000e6, 0)
    ...
    l = @layout[
        ...
        p_sum = Plots.plot(
            plot(paleorun.output, "ocean.corg_bf", label=false, xlabel="", ylims=(0.0, 2.5)),
            ...
            plot(paleorun.output, "oceanfloor.anoxia_burial_frac", label=false),
            layout = l, 
            left_margin = 5Plots.mm, bottom_margin = 1Plots.mm,
            size=(800, 800)
        )
        display(p_sum)
        savefig(p_sum, joinpath(output_figures_dir, "POAU_figure5_change_regimes_time_series.svg"))
    # ###########################################################
...
# # # ####### For the 2D mapping part #######
# function plot_3D_to_2D_mappings(exptname)
#     (;paleorun, modeldata, t_ts, A_ts, P_ts, O_ts, A_val, O_val,
#     dPdt_surf, dAdt_surf, dOdt_surf,
#     dPdt_linesegs_const_A, Obal_P, Obal_O, 
#     dPdt_line_const_O, dAdt_line_const_O, dPdt_linesegs_const_O,
#     folds_dPdt_lines_POA, dOdt_dPdt_linesegs_POA, dAdt_dPdt_linesegs_POA, dAdt_dPdt_linesegs_POA_end, eqb_point) = 
#         P_O_A_U_columns_Table2_3D_sum[exptname]
...
#     Plots.scatter!(plot_PO, [eqb_point[1]], [eqb_point[2]]; color=:red, markershape=:circle, markerstrokewidth=0, label=nothing);

#     #Plots.plot!(plot_OA, O_ts, A_ts; color=:green, linestyles=linestyles[1], label=false)
#     Plots.scatter!(plot_OA, [eqb_point[2]], [eqb_point[3]]; color=:red, markershape=:circle, markerstrokewidth=0, label=nothing);

#     # Plots.plot!(plot_PA, P_ts, A_ts; color=:green, linestyles=linestyles[1], label=false)
...
#     p_sum = Plots.plot(plot_PO, plot_OA, plot_PA,
#                      layout = l, left_margin = 5Plots.mm, bottom_margin = 5Plots.mm, size=(700, 250))

#     savefig(p_sum, joinpath(output_figures_dir, exptname*"2D_mapping_20240131.svg"))
# end

# plot_3D_to_2D_mappings("preCambrian_Bergman_Psilw_only")
# plot_3D_to_2D_mappings("preCambrian_Bergman_sharp_switch_unstable5_Corgb4_Psilw_only")
