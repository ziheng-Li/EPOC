...
output_figures_dir = joinpath(dropbox_output_dir, "P_O_A_U_Table7_white_noise_change_regimes_20241219")
...
# NB: new 3rd argument 1 or 2 to select tspan[1] or tspan[2] for phase-plane plots
expts_table = [

    ("POA 2010", "preCambrian_Bergman_sharp_switch_unstable5_Corgb2_Psilw_only_CO2_whitenoise", 2,
        [
            ...
            ("set_interp_forcing", "ocean", "force_corg_bf",    
                [0.0, 1e8, 1e8+1, 2e8, 2e8+1, 3e8, 3e8+1, 4e8, 4e8+1, 5e8],
                # [0.9, 0.9, 1.1, 1.1, 1.12, 1.12, 1.2, 1.2, 1.3, 1.3],
                # SJD 2024-12-19 attempt to reproduce submitted figure
                [0.9, 0.9, 1.1, 1.1, 1.15, 1.15, 1.2, 1.2, 1.3, 1.3],
            ),
        ]
    ),

    ("POA 2011", "preCambrian_Bergman_sharp_switch_unstable5_Corgb5_Psilw_only_CO2pulse_whitenoise", 1,
        [
            ("P_weathering", 3.9e10, 1.0, 0.0, 0.0), 
            # ("CO2pulse", age, pulse), 
            ...
            ("set_interp_forcing", "ocean", "force_corg_bf",
                [0.0, 1e8, 1e8+1, 2e8, 2e8+1, 3e8, 3e8+1, 4e8, 4e8+1, 5e8],
                # [0.4, 0.4, 0.45, 0.45, 0.485, 0.485, 0.5, 0.5, 0.7, 0.7],
                # SJD 2024-12-19 attempt to reproduce submitted figure
                # [0.4, 0.4, 0.45, 0.45, 0.48, 0.48, 0.5, 0.5, 0.7, 0.7],
                # SJD 2024-12-19 further adjustment when setting CGconstant=true
                [0.4, 0.4, 0.45, 0.45, 0.475, 0.475, 0.5, 0.5, 0.7, 0.7]
            ),
        ]
    ),
...
t_omit_spinup = 1e7 # yr, initial spinup to omit from plots
t_eqb_points = [0.5e8, 1.5e8, 2.5e8, 3.5e8, 4.5e8] # yr, times to show eqb points (~mid-point time of each regime)

P_O_A_U_columns_Table4_1 = Dict()

for (expt_id, fileroot, phase_plane_tspan_idx, vector_pars) in expts_table

# (expt_id, fileroot, phase_plane_tspan_idx, vector_pars) = expts_table[1]
    ...
    # for i in [1]
        ...
        # omit spinup time
        ts_idx_spinup = findfirst(t->t > (t_omit_spinup+tspan[1]), t_ts)
        @info "omitting spinup before $(t_omit_spinup+tspan[1]) yr, first index used $ts_idx_spinup"
        t_ts = PB.get_data(paleorun.output, "global.tforce")
        ...
        # phase_plane_tspan_idx is used to select either beginning tspan[1] or end tspan[2] of time interval
        poand = SolverFunctionsOOEOAE2.POAstatenormDeriv(
            model, modeldata; 
            fixed_values=initial_state, fixed_t=tspan[phase_plane_tspan_idx], include_jacobian
        )
        if include_jacobian
        ...
        dOdt_dPdt_line_POA = only(SolverFunctionsOOEOAE2.Isoline.find_isolines(
            (P_n, O_n, A_n)->poand(P_n, O_n, A_n)[2], dPdt_surf)
        )
        # split at folds
        ...
        # find eqb points for each regime using Newton's method 
        eqb_points = []
        for t_eqb in t_eqb_points
            poand_eqb = SolverFunctionsOOEOAE2.POAstatenormDeriv(
                model, modeldata; 
                fixed_values=initial_state, fixed_t=t_eqb, include_jacobian=true,
            )
            eqb_point_newton = SolverFunctionsOOEOAE2.find_intersection_point_newton(poand_eqb)
            @info "eqb_point_newton t=$t_eqb POA: $eqb_point_newton"
            push!(eqb_points, eqb_point_newton)
        end
        ...
        # add limit cycle time series, omitting spinup
        GLMakie.lines!(
            axs,
            P_ts[ts_idx_spinup:end], O_ts[ts_idx_spinup:end], A_ts[ts_idx_spinup:end];
            color=:green, linestyle=:solid, linewidth=glm_linewidth, # linewidth=2*glm_linewidth,
        )

        GLMakie.scatter!(axs, eqb_points; color=:red, markersize=glm_eqb_markersize,)
        # show first and last points
        # GLMakie.scatter!(axs, (P_ts[ts_idx_spinup], O_ts[ts_idx_spinup], A_ts[ts_idx_spinup]); color=:blue, markersize=glm_eqb_markersize,)
...
display(p_sum)
savefig(p_sum, joinpath(output_figures_dir, "Whitenoise_oxic_anoxic_sum_20241219.svg"))