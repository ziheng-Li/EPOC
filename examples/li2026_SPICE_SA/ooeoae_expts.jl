...
function ooeoae_expts(model, expts)
    ...
    for expt in expts
        if length(expt) == 4 && expt[1] == "set_initial_value"
        ...
        elseif length(expt) == 5 && expt[1] == "set_parameter"
            # generic parameter set (set_parameter, <domain>, <reaction>, <parname>, <value>)
            _, domname, reaction, parname, value = expt            
            PB.set_parameter_value!(model, domname, reaction, parname, value)

        elseif length(expt) == 2 && first(expt) == "OPinit"
        ...
        elseif length(expt) == 2 && first(expt) == "POAinit"
            _, (P_init, O_init, A_init) = expt
            PB.set_variable_attribute!(model, "ocean.P", :initial_value, 3.1e15*P_init)
            PB.set_variable_attribute!(model, "atmocean.O", :initial_value, 3.7e19*O_init)
            PB.set_variable_attribute!(model, "atmocean.A", :initial_value, 3.193e18*A_init)

        elseif length(expt) == 2 && first(expt) == "POASinit"
            _, (P_init, O_init, A_init, S_init) = expt
            PB.set_variable_attribute!(model, "ocean.P", :initial_value, 3.1e15*P_init)
            PB.set_variable_attribute!(model, "atmocean.O", :initial_value, 3.7e19*O_init)
            PB.set_variable_attribute!(model, "atmocean.A", :initial_value, 3.193e18*A_init)
            PB.set_variable_attribute!(model, "ocean.S", :initial_value, 4e19*S_init)
        ...
        elseif length(expt) == 2 && first(expt) == "k_fe2"
            _, k_fe2 = expt
            PB.set_parameter_value!(model, "ocean", "oceanburial_columns", "k_fe2", k_fe2)
        ...
        elseif length(expt) == 4 && expt[1] == "extraC_from_DOC"
            _, CO2times, CO2fluxes,  CO2delta= expt
            CO2times_all = vcat([-1e30, first(CO2times)-1.0], CO2times, [last(CO2times)+1.0, 1e30])
            CO2fluxes_all = vcat([0.0, 0.0], CO2fluxes, [0.0, 0.0])
            CO2delta_all = vcat([-5.0,-5.0], CO2delta, [-5.0,-5.0])
            @info "$(CO2times_all)"
            @info "$(CO2fluxes_all)"
            PB.set_parameter_value!(model, "global", "CO2pulse", "perturb_times", CO2times_all)
            PB.set_parameter_value!(model, "global", "CO2pulse", "perturb_totals", CO2fluxes_all)
            PB.set_parameter_value!(model, "global", "CO2pulse", "perturb_deltas", CO2delta_all) 
        ...
        elseif length(expt)==5 && expt[1] == "set_parameter"
            _, domian, reaction, par_name, par_value = expt #
            PB.set_parameter_value!(model, domian, reaction, par_name, par_value)

        elseif length(expt)==4 && expt[1] == "VEG"
            _, pstart, pend, size = expt #
            force_VEG = PB.get_reaction(model, "land", "force_VEG")
            PB.setvalue!(PB.get_parameter(force_VEG, "force_times"),  
                [-1e30, pstart-1, pstart,  pend, pend+1, 1e30]) 
            PB.setvalue!(PB.get_parameter(force_VEG, "force_values"), 
                [1.0,   1.0,    size,    size, 1.0, 1.0])

        elseif length(expt)==5 && expt[1] == "VEG_gradually_recover"
            _, pstart, pmid, pend, size = expt #
            force_VEG = PB.get_reaction(model, "land", "force_VEG")
            PB.setvalue!(PB.get_parameter(force_VEG, "force_times"),  
                [-1e30, pstart-1, pstart, pmid, pend, pend+1, 1e30]) 
            PB.setvalue!(PB.get_parameter(force_VEG, "force_values"), 
                [1.0,   1.0,    size,  size,  1.0, 1.0, 1.0])

        elseif expt[1] == "VEG_recover_model"
            force_VEG = PB.get_reaction(model, "land", "force_VEG")
            PB.setvalue!(PB.get_parameter(force_VEG, "force_times"),  
                [-1e30, -252e6-1, -252e6, -250e6, -247.0e6-1, -247.0e6, -237e6, 1e30]) 
            PB.setvalue!(PB.get_parameter(force_VEG, "force_values"), 
                [1.0,   1.0,      0.0,    0.0,    0.20,      0.30,      0.30,    1.0]) 
        
        elseif length(expt)==3 && expt[1] == "VEG_manual_input"
            _, force_times, force_values = expt #
            force_VEG = PB.get_reaction(model, "land", "force_VEG")
            PB.setvalue!(PB.get_parameter(force_VEG, "force_times"),  force_times) 
            PB.setvalue!(PB.get_parameter(force_VEG, "force_values"), force_values)

        elseif length(expt)==4 && expt[1] == "VEG_HMC_3pars"
            _, start_recovery, end_recovery, Max_recovery = expt #
            force_VEG = PB.get_reaction(model, "land", "force_VEG")
            PB.setvalue!(PB.get_parameter(force_VEG, "start_recovery"), start_recovery) 
            PB.setvalue!(PB.get_parameter(force_VEG, "end_recovery"), end_recovery)
            PB.setvalue!(PB.get_parameter(force_VEG, "Max_recovery"), Max_recovery)
        
        elseif length(expt)==3 && expt[1] == "VEG_HMC_2pars"
            _, end_recovery, Max_recovery = expt #
            force_VEG = PB.get_reaction(model, "land", "force_VEG") 
            PB.setvalue!(PB.get_parameter(force_VEG, "end_recovery"), end_recovery)
            PB.setvalue!(PB.get_parameter(force_VEG, "Max_recovery"), Max_recovery)

        elseif length(expt)==6 && expt[1] == "VEG_HMC_5pars"
            _, end_extinct, Min_extinct, start_recovery, end_recovery, Max_recovery = expt #
            force_VEG = PB.get_reaction(model, "land", "force_VEG") 
            PB.setvalue!(PB.get_parameter(force_VEG, "end_extinct"), end_extinct)
            PB.setvalue!(PB.get_parameter(force_VEG, "Min_extinct"), Min_extinct)
            PB.setvalue!(PB.get_parameter(force_VEG, "start_recovery"), start_recovery)
            PB.setvalue!(PB.get_parameter(force_VEG, "end_recovery"), end_recovery)
            PB.setvalue!(PB.get_parameter(force_VEG, "Max_recovery"), Max_recovery)

        elseif length(expt)==8 && expt[1] == "noderiv_5pars"
            _, domian, reaction, end_extinct, Min_extinct, start_recovery, end_recovery, Max_recovery = expt #
            force_VEG = PB.get_reaction(model, domian, reaction) 
            PB.setvalue!(PB.get_parameter(force_VEG, "end_extinct"), end_extinct)
            PB.setvalue!(PB.get_parameter(force_VEG, "Min_extinct"), Min_extinct)
            PB.setvalue!(PB.get_parameter(force_VEG, "start_recovery"), start_recovery)
            PB.setvalue!(PB.get_parameter(force_VEG, "end_recovery"), end_recovery)
        ...
        elseif length(expt)==2 && expt[1] == "VEG_HMC_npars"
            VEG_times = collect(expt[2]) #
            force_VEG = PB.get_reaction(model, "land", "force_VEG") 
            # PB.setvalue!(force_VEG.pars.force_times, vcat([-254e6, -252e6-1], VEG_times))
            # PB.setvalue!(force_VEG.pars.force_values, vcat([1.0, 1.0], zeros(length(VEG_times))))
            PB.setvalue!(force_VEG.pars.force_times_vcat, VEG_times)
            PB.setvalue!(force_VEG.pars.force_values_vcat, zeros(length(VEG_times)))

        elseif length(expt)==4 && expt[1] == "noderiv_npars"
            _, domian, reaction, _ = expt #
            VEG_times = collect(expt[4]) #
            force_VEG = PB.get_reaction(model, domian, reaction) 
            PB.setvalue!(force_VEG.pars.force_times_vcat, VEG_times)
            PB.setvalue!(force_VEG.pars.force_values_vcat, zeros(length(VEG_times)).+0.1)

        elseif length(expt)==4 && expt[1] == "Uplift"
            _, pstart, pend, size = expt #
            force_UPLIFT = PB.get_reaction(model, "global", "force_UPLIFT")
            PB.setvalue!(PB.get_parameter(force_UPLIFT, "force_times"),  
                [-1e30, pstart-1, pstart, pend, pend+1, 1e30]) 
            PB.setvalue!(PB.get_parameter(force_UPLIFT, "force_values"), 
                [0.55,   0.55,    size,  size,  0.55, 0.55])

        elseif length(expt)==5 && expt[1] == "Uplift_gradually_recover"
            _, pstart, pend, size, sizeend = expt #
            force_UPLIFT = PB.get_reaction(model, "global", "force_UPLIFT")
            PB.setvalue!(PB.get_parameter(force_UPLIFT, "force_times"),  
                [-1e30, pstart-1, pstart, pend, pend+1, 1e30]) 
            PB.setvalue!(PB.get_parameter(force_UPLIFT, "force_values"), 
                [0.55,   0.55,    size,  sizeend,  sizeend, sizeend])

        elseif length(expt)==5 && expt[1] == "kO2U_gradually_recover"
            _, pstart, pend, size, sizeend = expt #
            force_kO2U = PB.get_reaction(model, "ocean", "force_kO2U")
            PB.setvalue!(PB.get_parameter(force_kO2U, "force_times"),  
                [-1e30, pstart-1, pstart, pend, pend+1, 1e30]) 
            PB.setvalue!(PB.get_parameter(force_kO2U, "force_values"), 
                [1.0,   1.0,    size,  sizeend,  sizeend, sizeend])

        elseif length(expt)==3 && expt[1] == "kO2U_manual_input"
            _, force_times, force_values = expt #
            force_kO2U = PB.get_reaction(model, "ocean", "force_kO2U")
            PB.setvalue!(PB.get_parameter(force_kO2U, "force_times"),  force_times) 
            PB.setvalue!(PB.get_parameter(force_kO2U, "force_values"), force_values)
        
        elseif length(expt) == 3 && first(expt) == "set_SHELF_AREA_NORM"
            _, satimes, savalues = expt
            PB.set_parameter_value!(model, "global", "force_SHELF_AREA_NORM", "force_times", satimes)
            PB.set_parameter_value!(model, "global", "force_SHELF_AREA_NORM", "force_values", savalues)
        
        elseif expt == "set_forcing_EndP"
            PB.set_variable_attribute!(model, "global", "PG", :initial_value, 0.54) # 0.54
            PB.set_variable_attribute!(model, "global", "DEGASS", :initial_value, 1.13)
            PB.set_variable_attribute!(model, "global", "COAL", :initial_value, 4.07) # 4.07
            # PB.set_variable_attribute!(model, "global", "UPLIFT", :initial_value, 0.55) # 0.55 is changed to ReactionForceInterp

            PB.set_variable_attribute!(model, "land", "BA_AREA", :initial_value, 0.7) # 0.70
            PB.set_variable_attribute!(model, "land", "GRAN_AREA", :initial_value, 1.56) # 1.56
            ...
            PB.set_variable_attribute!(model, "ocean", "P", :initial_value, 3.1e15*1.0)
        
        elseif expt == "set_forcing_EndP_0915"
            # PG = 0.54 in COPSEreload, Stuart suggested to remove the forcing PG
            PB.set_variable_attribute!(model, "global", "PG", :initial_value, 1.0) # 0.54
            PB.set_variable_attribute!(model, "global", "DEGASS", :initial_value, 1.13)
            PB.set_variable_attribute!(model, "global", "COAL", :initial_value, 1.0) # 4.07
            # PB.set_variable_attribute!(model, "global", "UPLIFT", :initial_value, 0.55) # 0.55 is changed to ReactionForceInterp
        ...
        elseif expt == "set_forcing_EndT"
            # PG = 0.54 in COPSEreload, Stuart suggested to remove the forcing PG
            PB.set_variable_attribute!(model, "global", "PG", :initial_value, 1.0)
            PB.set_variable_attribute!(model, "global", "DEGASS", :initial_value, 1.3)
            PB.set_variable_attribute!(model, "global", "COAL", :initial_value, 0.5)
            # PB.set_variable_attribute!(model, "global", "UPLIFT", :initial_value, 0.55) # 0.55 is changed to ReactionForceInterp

            PB.set_variable_attribute!(model, "land", "BA_AREA", :initial_value, 1.0)
            PB.set_variable_attribute!(model, "land", "GRAN_AREA", :initial_value, 1.0) 
            PB.set_variable_attribute!(model, "land", "CARB_AREA", :initial_value, 1.0)
            PB.set_variable_attribute!(model, "land", "VEG", :initial_value, 1.0)

            PB.set_variable_attribute!(model, "atmocean", "A", :initial_value, 3.193e18*7.14^0.5)    # 2000 ppm
            PB.set_variable_attribute!(model, "atmocean", "O", :initial_value, 3.7e19*1.143)        # 0.24%
            PB.set_variable_attribute!(model, "ocean", "P", :initial_value, 3.1e15*1.0)


        elseif expt == "set_forcing_Mesoproterozic"
            # PG = 0.54 in COPSEreload, Stuart suggested to remove the forcing PG
            PB.set_variable_attribute!(model, "global", "PG", :initial_value, 1.0) # 0.54
            PB.set_variable_attribute!(model, "global", "DEGASS", :initial_value, 1.13)
            PB.set_variable_attribute!(model, "global", "COAL", :initial_value, 1.0) # 4.07
        ...
        elseif expt == "set_forcing_EndO"
            # set a end Ordovician steady state
            PB.set_variable_attribute!(model, "global", "PG", :initial_value, 0.72) # 0.72
            PB.set_variable_attribute!(model, "global", "DEGASS", :initial_value, 1.51) # 1.51
            PB.set_variable_attribute!(model, "global", "COAL", :initial_value, 1.0) # 4.07
            # PB.set_variable_attribute!(model, "global", "UPLIFT", :initial_value, 0.97) # 0.97 is changed to ReactionForceInterp

            PB.set_variable_attribute!(model, "land", "BA_AREA", :initial_value, 0.62) # 0.62
            PB.set_variable_attribute!(model, "land", "GRAN_AREA", :initial_value, 1.87) # 1.87
            PB.set_variable_attribute!(model, "land", "CARB_AREA", :initial_value, 1.0)
            PB.set_variable_attribute!(model, "land", "VEG", :initial_value, 0.38) # 0.38
            # carbonate cycle is simplified, so CARB_AREA is not changed here

            PB.set_variable_attribute!(model, "atmocean", "A", :initial_value, 3.193e18*7.5^0.5)
            PB.set_variable_attribute!(model, "atmocean", "O", :initial_value, 3.7e19*0.35)
            PB.set_variable_attribute!(model, "ocean", "P", :initial_value, 3.1e15*1.0)
        ...
        elseif length(expt) == 2 && expt[1] == "U_weathering_only"
            _, perturb_totals = expt
            PB.set_parameter_value!(model, "land", "U_land", "k_U_total_silw", 0.0) # no silicate-weathering-dependent U weathering
            PB.set_parameter_value!(model, "land", "U_weathering", "perturb_totals", [perturb_totals, perturb_totals]) # add constant U weathering
        ...
        elseif length(expt) == 4 && expt[1] == "COPSE_locb"
            _, CPland0, k5_locb, k11_landfrac = expt
            PB.set_parameter_value!(model, "land", "land_fluxes", "CPland0", CPland0)
            PB.set_parameter_value!(model, "land", "land_fluxes", "k5_locb", k5_locb)
            PB.set_parameter_value!(model, "land", "land_fluxes", "k11_landfrac", k11_landfrac)
        ...
        elseif length(expt) == 5 && expt[1] == "SFe_pars"
            _, SFe_low, SFe_up, k_eux, k_fe2w = expt
            PB.set_parameter_value!(model, "ocean", "oceanburial_columns", "SFe_low", SFe_low)
            PB.set_parameter_value!(model, "ocean", "oceanburial_columns", "SFe_up", SFe_up)
            PB.set_parameter_value!(model, "ocean", "oceanburial_columns", "k_eux", k_eux)
            PB.set_parameter_value!(model, "ocean", "oceanburial_columns", "k_fe2w", k_fe2w)
